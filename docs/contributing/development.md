...
git clone https://github.com/iOfficeAI/Trace.git
cd Trace
...
When you have two clones of the repository (e.g. `Trace` and `Trace-refactor`) and need to run both simultaneously, the second instance can be started with:

```bash
bun run start:multi
```

This sets `TRACE_MULTI_INSTANCE=1`, which:

- Skips the Electron single-instance lock
- Uses a separate userData directory (`Trace-Dev-2`) to avoid database and config conflicts
- Isolates data/config symlink paths (`~/.trace-dev-2`, `~/.trace-config-dev-2`)
...
Trace uses **electron-vite** for fast bundling:
