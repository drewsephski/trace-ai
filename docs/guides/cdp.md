...
Trace supports CDP for external debugging tools integration. In development mode (`just dev`), CDP is enabled by default on port 9230.

## Enable CDP in Production

1. Open Trace Settings → System → Developer Debug
...
Trace can integrate with other MCP tools for enhanced development experience:
...
Once configured, you can use MCP tools to interact with Trace:

- `list_pages` — List all open pages in Trace
...
Trace 支持 CDP 用于外部调试工具集成。在开发模式 (`just dev`) 下，CDP 默认在端口 9230 启用。

## 在生产环境启用 CDP

1. 打开 Trace 设置 → 系统 → 开发者调试
2. 启用"启用远程调试 (CDP)"
3. 重启应用
...
配置完成后，可以使用 MCP 工具与 Trace 交互：

- `list_pages` — 列出 Trace 中所有打开的页面
