...
describe('useGuidAssistantSelection', () => {
  ...
  it('hides internal Aion assistants from the guide selection catalog', async () => {
    mockAssistants = [
      {
        id: 'bare:632f31d2',
        source: 'generated',
  ...
  it('defaults to OpenCode ahead of other visible generated assistants', async () => {
    mockAssistants = [
      {
        id: 'assistant-claude',
        source: 'generated',
    ...
    expect(result.current.defaultAssistantId).toBe('assistant-opencode');
    expect(result.current.selectedAssistantBackend).toBe('opencode');
  });
});
