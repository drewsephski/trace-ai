...
describe('configMigrationIntegration', () => {
  ...
  it('runs migrations when database version is outdated', async () => {
    const result = await runLegacyDatabaseMigrations('/test/trace.db');
  ...
  it('closes driver after migration completes', async () => {
    await runLegacyDatabaseMigrations('/test/trace.db');
  ...
  it('ensures system user exists after migration', async () => {
    ...
    expect(mockDriver.prepare).toHaveBeenCalledWith(expect.stringContaining('username IN'));
  });
  ...
  it('closes driver even if migration throws', async () => {
    ...
    await expect(runLegacyDatabaseMigrations('/test/trace.db')).rejects.toThrow('Migration failed');
    expect(mockDriver.close).toHaveBeenCalled();
  });
});
