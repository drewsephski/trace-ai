...
vi.mock('electron', () => ({
  ...
  app: {
    getPath: vi.fn(() => '/tmp/trace-test-logs-nonexistent'),
...
describe('feedback logs', () => {
  it('collects top-level frontend logs and nested backend logs through the IPC handler', async () => {
    const logsDir = mkdtempSync(path.join(tmpdir(), 'trace-feedback-bridge-'));
    try {
  ...
  it('collects the same recent three log days used by user feedback reports', () => {
    const logsDir = mkdtempSync(path.join(tmpdir(), 'trace-feedback-logs-'));
    try {
