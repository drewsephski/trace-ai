...
const makeGitHubReleaseResponse = () => [
  {
    ...
    name: 'v1.9.22',
    body: 'release notes',
    html_url: 'https://github.com/drewsephski/trace-ai/releases/tag/v1.9.22',
    published_at: '2026-04-29T00:00:00Z',
    prerelease: false,
    draft: false,
    assets: [
      {
        name: 'Trace-1.9.22-mac-arm64.dmg',
        browser_download_url:
          'https://github.com/drewsephski/trace-ai/releases/download/v1.9.22/Trace-1.9.22-mac-arm64.dmg',
        size: 123,
        content_type: 'application/x-apple-diskimage',
      },
      {
        name: 'Trace-1.9.22-win-x64.exe',
        browser_download_url:
          'https://github.com/drewsephski/trace-ai/releases/download/v1.9.22/Trace-1.9.22-win-x64.exe',
        size: 456,
        content_type: 'application/vnd.microsoft.portable-executable',
      },
      {
        name: 'Trace-1.9.22-linux-amd64.deb',
        browser_download_url:
          'https://github.com/drewsephski/trace-ai/releases/download/v1.9.22/Trace-1.9.22-linux-amd64.deb',
        size: 789,
      },
    ],
  },
...
describe('updateBridge release asset URLs', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('uses GitHub Release asset URLs directly without an upstream CDN fallback', async () => {
    ...
    try {
      ...
      const result = await handler({ repo: 'drewsephski/trace-ai' });
      ...
      const macAsset = assets.find((a: { name: string }) => a.name === 'Trace-1.9.22-mac-arm64.dmg');
      ...
      const linuxAsset = assets.find((a: { name: string }) => a.name === 'Trace-1.9.22-linux-amd64.deb');
  ...
  it('keeps the GitHub tag URL untouched', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => makeGitHubReleaseResponse(),
    });
    ...
    try {
      ...
      expect(asset?.url).toMatch(/^https:\/\/github\.com\/drewsephski\/trace-ai\/releases\/download\/v1\.9\.22\//);
    } finally {
      vi.unstubAllGlobals();
    }
  });
...
describe('updateBridge allowlist includes GitHub release hosts', () => {
  it('accepts GitHub Release URLs for download', async () => {
    ...
    try {
      ...
      const result = await handler({
        ...
        url: 'https://github.com/drewsephski/trace-ai/releases/download/v1.9.22/Trace-1.9.22-mac-arm64.dmg',
        file_name: 'Trace-1.9.22-mac-arm64.dmg',
      });
