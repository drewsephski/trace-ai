...
describe('ensureAdminPassword', () => {
  it('seeds password on fresh install (needs_setup=true)', async () => {
    ...
    expect(logs).toContain('[trace-web] Generated initial admin password: SuperSecret123');
  ...
  it('accepts top-level new_password in reset-password response', async () => {
    ...
    expect(logs).toContain('[trace-web] Generated initial admin password: FromTopLevel');
  });
  ...
  it('falls back to "Drew" username when system user lookup fails', async () => {
    const { deps, logs } = makeDeps({
      handlers: [
        () => mockResponse(200, { needs_setup: true }),
        () => mockResponse(200, { data: { new_password: 'Pw' } }),
    ...
    expect(logs.some((m) => m.includes('Log in with username "Drew"'))).toBe(true);
  });
  ...
  it('uses caller-supplied resetCommand in the "Forgot the password" hint', async () => {
    ...
    expect(logs.every((m) => !m.includes('trace-web resetpass'))).toBe(true);
  });

  it('defaults to `trace-web resetpass` when resetCommand is not provided', async () => {
    const { deps, logs } = makeDeps({
      handlers: [
        () => mockResponse(200, { needs_setup: false }),
        () => mockResponse(200, { data: { username: 'admin' } }),
    ...
    expect(logs.some((m) => m.includes('trace-web resetpass'))).toBe(true);
  });
