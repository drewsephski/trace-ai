...
describe('UpdateModal manual install fallback', () => {
  beforeEach(() => {
    ...
    mocks.updateCheckMock.mockResolvedValue({
      ...
      data: {
        ...
        latest: {
          ...
          name: 'v2.1.14',
          body: 'notes',
          htmlUrl: 'https://github.com/iOfficeAI/Trace/releases/tag/v2.1.14',
          prerelease: false,
          draft: false,
          assets: [],
          recommendedAsset: {
            name: 'Trace-2.1.14-mac-arm64.dmg',
            url: 'https://static.trace.com/releases/2.1.14/Trace-2.1.14-mac-arm64.dmg',
            fallbackUrl: 'https://github.com/iOfficeAI/Trace/releases/download/v2.1.14/Trace-2.1.14-mac-arm64.dmg',
            size: 123,
          },
        },
      },
    ...
    mocks.updateDownloadMock.mockImplementation(async (request: UpdateDownloadRequest) => {
      ...
      mocks.manualProgressHandler?.({
        ...
        totalBytes: 123,
        percent: 100,
        file_path: '/tmp/Trace-2.1.14-mac-arm64.dmg',
      });
      return {
        success: true,
        data: {
  ...
  it('keeps fast manual download completion matched to the caller-provided download id', async () => {
    ...
    act(() => {
      window.dispatchEvent(new Event('trace-open-update-modal'));
    });
    ...
    expect(mocks.updateDownloadMock).toHaveBeenCalledWith({
      ...
      url: 'https://static.trace.com/releases/2.1.14/Trace-2.1.14-mac-arm64.dmg',
      fallbackUrl: 'https://github.com/iOfficeAI/Trace/releases/download/v2.1.14/Trace-2.1.14-mac-arm64.dmg',
      file_name: 'Trace-2.1.14-mac-arm64.dmg',
    });
