...
```typescript
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'trace-e2e-state-'));
...
```typescript
TRACE_EXTENSION_STATES_FILE: process.env.TRACE_EXTENSION_STATES_FILE || e2eStateFile;
```
...
**Critical bottleneck**: `workers: 1` enforced + shared `trace.db` → parallel execution impossible without refactoring.
...
1. Extend `TRACE_E2E_TEST` to include worker ID:
   ```typescript
   TRACE_E2E_TEST_WORKER_ID: process.env.PLAYWRIGHT_WORKER_INDEX || '0';
   ```
2. Modify `getDevAppName()` to return worker-specific name:
   ```typescript
   const workerId = process.env.TRACE_E2E_TEST_WORKER_ID || '0';
   return `Trace-E2E-Worker-${workerId}`;
   ```
3. Each worker gets isolated:
   - `~/Library/Application Support/Trace-E2E-Worker-0/config/trace.db`
   - `~/Library/Application Support/Trace-E2E-Worker-1/config/trace.db`
...
**Complexity**: High (requires main process to read `TRACE_DATABASE_PATH`, conflicts with userData convention)
