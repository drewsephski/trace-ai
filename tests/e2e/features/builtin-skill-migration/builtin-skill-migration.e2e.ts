 ...
 */
const REMOVED_AUTO_INJECT_NAME = 'trace-skills';
const REMOVED_AUTO_INJECT_DIR_NAME = 'trace-skills';
...
function resolveBackendBinary(): string {
  const candidates = [process.env.TRACE_BACKEND_BINARY, path.join(os.homedir(), '.cargo', 'bin', 'aioncore')].filter(
    (x): x is string => typeof x === 'string' && x.length > 0
  );
  for (const c of candidates) {
    if (fs.existsSync(c)) return c;
  }
  throw new Error('aioncore binary not found. Set TRACE_BACKEND_BINARY or install to ~/.cargo/bin/aioncore.');
}
...
test.describe('Built-in Skill Migration (T3)', () => {
  ...
  test('S7: builtin skills in /api/skills expose an absolute, readable location for export', async ({ page }) => {
    ...
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'trace-e2e-s7-export-'));
    try {
  ...
  test.describe('Cold-start invariants (sibling backend)', () => {
    ...
    async function startBackend(): Promise<void> {
      ...
      delete parentEnv.TRACE_EXTENSIONS_PATH;
      delete parentEnv.TRACE_EXTENSION_STATES_FILE;
      delete parentEnv.TRACE_E2E_TEST;
      delete parentEnv.TRACE_CDP_PORT;
      delete parentEnv.TRACE_BUILTIN_SKILLS_PATH;
    ...
    test.beforeEach(() => {
      dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'trace-e2e-builtin-skill-'));
    });
    ...
    test('S8: legacy {cacheDir}/builtin-skills/ is gone after the current Electron boot', async () => {
      ...
      // `~/.trace-config` tree. Failing that we at least assert the
      ...
      const candidates = [
        path.join(os.homedir(), '.trace-config', 'builtin-skills'),
        path.join(os.homedir(), '.trace-config-dev', 'builtin-skills'),
      ];
      ...
      test.info().annotations.push({
        ...
        description:
          survivors.length === 0
            ? 'no legacy builtin-skills cache dirs detected under ~/.trace-config*'
