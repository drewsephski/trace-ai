 ...
 * 1. seed a legacy `trace.db` using the pre-unification assistants schema
...
function schemaPath(): string {
  return path.join(process.cwd(), '../aionCore/crates/trace-db/migrations/001_initial_schema.sql');
}
...
function seedLegacyDatabase(dataDir: string): void {
  const legacyDbPath = path.join(dataDir, 'trace.db');
...
test.describe('Assistant Settings Migration', () => {
  ...
  async function startBackend(): Promise<void> {
    ...
    backend = spawn(binary, ['--local', '--port', String(MIGRATION_BACKEND_PORT), '--data-dir', dataDir], {
      ...
      env: {
        ...process.env,
        RUST_LOG: 'warn',
        TRACE_EXTENSIONS_PATH: path.join(process.cwd(), 'examples'),
      },
    });
    try {
  ...
  test.beforeEach(async () => {
    dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'trace-assistant-migration-'));
