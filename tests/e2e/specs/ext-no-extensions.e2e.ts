...
const emptyExtensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'trace-e2e-no-extensions-'));
const stateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'trace-e2e-no-extensions-state-'));
...
async function launchAppWithoutExtensions(): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      TRACE_EXTENSIONS_PATH: emptyExtensionsDir,
      TRACE_EXTENSION_STATES_FILE: extensionStatesFile,
