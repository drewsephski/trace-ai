 ...
 *      (e.g. out/linux-unpacked/trace, out/mac-arm64/Trace.app, out/win-unpacked/Trace.exe).
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/Trace.exe  or  out/win-x64-unpacked/Trace.exe
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, 'Trace.exe');
      if (fs.existsSync(exe)) return { executablePath: exe, cwd: path.join(outDir, dir) };
    }
  } else if (platform === 'darwin') {
    // out/mac-arm64/Trace.app/Contents/MacOS/Trace  or  out/mac/Trace.app/...
...
async function launchApp(): Promise<ElectronApplication> {
  ...
  const commonEnv = {
    ...process.env,
    TRACE_EXTENSIONS_PATH: process.env.TRACE_EXTENSIONS_PATH || path.join(projectRoot, 'examples'),
