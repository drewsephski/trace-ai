 ...
 * (`/Users/zhoukai/Downloads/trace-config.txt` + `Archive/*.md`).
 *
 ...
 *
 *   1. Decode the legacy `trace-config.txt` exactly as ConfigStorage would.
...
const FIXTURE_CONFIG = '/Users/zhoukai/Downloads/trace-config.txt';
...
function resolveBackendBinary(): string {
  const candidates = [
    process.env.TRACE_BACKEND_BINARY,
  ...
  throw new Error('aioncore binary not found (set TRACE_BACKEND_BINARY or build it)');
}
...
/**
 * Decode an `trace-config.txt` exactly the way `JsonFileBuilder` does:
...
describeIfFixtures('migrateAssistantsToBackend (real fixture)', () => {
  ...
  beforeEach(async () => {
    dataDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'trace-migrate-fixture-'));
