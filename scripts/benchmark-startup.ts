 ...
 * parsing the electron-log file for [Trace:ready] / [Trace:init] /
 * [Trace:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  // Parsed from [Trace:init] marks
  initTotalMs: number;
  // Parsed from [Trace:process] marks
...
function getLogFilePath(): string {
  ...
  if (process.platform === 'darwin') {
    candidates.push(
      path.join(os.homedir(), 'Library', 'Logs', 'Trace-Dev', `${today}.log`),
      path.join(os.homedir(), 'Library', 'Logs', 'Trace', `${today}.log`)
    );
  } else if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    candidates.push(path.join(appData, 'Trace', 'logs', `${today}.log`));
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'Trace', 'logs', `${today}.log`));
  }
...
// Matches: [Trace:ready] <label> +<ms>ms
// Matches: [Trace:init]  <label> +<ms>ms
// Matches: [Trace:process] <label> +<ms>ms
const MARK_REGEX = /\[Trace:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[Trace] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[Trace] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[Trace] Showing main window')) marks.logs.showingMainWindow = true;
  }
...
async function launchApp(timeoutMs: number, withMemory: boolean): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      TRACE_DISABLE_AUTO_UPDATE: '1',
      TRACE_E2E_TEST: '1',
      TRACE_DISABLE_DEVTOOLS: '1',
      TRACE_CDP_PORT: '0',
      NODE_ENV: 'production',
    },
    timeout: timeoutMs,
