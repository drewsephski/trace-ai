...
#   5. 建立服務管理腳本 (/opt/Trace/start-trace.sh)
#   6. (可選) 建立 systemd service
#   7. (可選) 建立桌面捷徑
#
# 用法：
#   curl -fsSL https://raw.githubusercontent.com/drewsephski/trace-ai/main/scripts/install-ubuntu.sh | bash
#   # 或指定版本：
#   TRACE_VERSION=1.8.25 bash install-ubuntu.sh
#   # 僅安裝桌面版（跳過 headless 設定）：
#   TRACE_MODE=desktop bash install-ubuntu.sh
# ============================================================================
...
banner() {
    echo -e "${CYAN}${BOLD}"
    echo "  ╔══════════════════════════════════════════════╗"
    echo "  ║          Trace Installer for Ubuntu          ║"
    echo "  ╚══════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
resolve_version() {
    if [[ -n "${TRACE_VERSION:-${TRACE_VERSION:-}}" ]]; then
        ...
        if command -v curl &>/dev/null; then
            VERSION=$(curl -fsSL "https://api.github.com/repos/drewsephski/trace-ai/releases/latest" \
                | grep '"tag_name"' | head -1 | sed 's/.*"v\([^"]*\)".*/\1/')
        elif command -v wget &>/dev/null; then
            VERSION=$(wget -qO- "https://api.github.com/repos/drewsephski/trace-ai/releases/latest" \
                | grep '"tag_name"' | head -1 | sed 's/.*"v\([^"]*\)".*/\1/')
        else
        ...
        if [[ -z "$VERSION" ]]; then
            die "無法取得最新版本號，請手動指定: TRACE_VERSION=1.8.25 bash $0"
        fi
        info "最新版本: ${BOLD}v$VERSION${NC}"
    fi

    DEB_FILENAME="Trace-${VERSION}-linux-${DEB_ARCH}.deb"
    DOWNLOAD_URL="https://github.com/drewsephski/trace-ai/releases/download/v${VERSION}/${DEB_FILENAME}"
}
...
install_deb() {
    ...
    success "Trace v${VERSION} 安裝完成"

    # 驗證安裝
    if command -v Trace &>/dev/null || [[ -x /usr/bin/Trace ]]; then
        success "Trace 已安裝至 $(which Trace 2>/dev/null || echo '/usr/bin/Trace')"
    else
        warn "安裝可能不完整，找不到 Trace 執行檔"
    fi
...
create_service_script() {
    local script_dir="/opt/Trace"
    local script_path="${script_dir}/start-trace.sh"
...
#!/bin/bash
# ============================================================================
# Trace WebUI Headless 服務管理腳本
# 用法: ./start-trace.sh [start|stop|restart|status|logs]
# ============================================================================
...
start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat "$PIDFILE")" 2>/dev/null; then
        echo "⚡ Trace 已在執行中 (PID: $(cat "$PIDFILE"))"
        return 1
    fi

    echo "🚀 正在啟動 Trace WebUI..."
    cd "$WORKDIR" || exit 1
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat "$PIDFILE")" 2>/dev/null; then
        echo "✅ Trace 執行中 (PID: $(cat "$PIDFILE"))"
...
case "${1:-}" in
    ...
    "")
        ...
        echo ""
        echo "環境變數:"
        echo "  TRACE_WORKDIR   - Trace 工作目錄 (預設: \$HOME)"
        ;;
    *)
        echo "用法: $0 {start|stop|restart|status|logs}"
        exit 1
...
create_systemd_service() {
    ...
    local service_path="/etc/systemd/system/trace.service"

    info "建立 systemd 服務: $service_path"

    $SUDO tee "$service_path" > /dev/null << 'SERVICE_EOF'
[Unit]
Description=Trace AI Agent Desktop App (WebUI Mode)
Documentation=https://github.com/drewsephski/trace-ai
...
SERVICE_EOF
    ...
    info "使用方式:"
    echo "    sudo systemctl start trace     # 啟動"
    echo "    sudo systemctl stop trace      # 停止"
    echo "    sudo systemctl enable trace    # 開機自動啟動"
    echo "    sudo systemctl status trace    # 查看狀態"
    echo "    journalctl -u trace -f         # 查看日誌"
}

# ─── 建立桌面捷徑 ─────────────────────────────────────────────────────────────
create_desktop_entry() {
    local desktop_dir="${HOME}/.local/share/applications"
    local desktop_file="${desktop_dir}/trace.desktop"
...
Exec=/usr/bin/Trace --no-sandbox %U
Icon=Trace
Terminal=false
Type=Application
Categories=Office;Utility;Development;
MimeType=x-scheme-handler/trace;
...
print_summary() {
    ...
    echo -e "${GREEN}${BOLD}  🎉 Trace v${VERSION} 安裝完成！${NC}"
    echo -e "${GREEN}${BOLD}══════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "  ${BOLD}📍 執行檔位置:${NC}  /usr/bin/Trace"
    echo -e "  ${BOLD}📍 管理腳本:${NC}    /opt/Trace/start-trace.sh"
    echo ""
    ...
    if [[ "${MODE}" == "headless" ]]; then
        ...
        echo ""
        echo "    # 使用管理腳本"
        echo "    /opt/Trace/start-trace.sh start"
        echo "    /opt/Trace/start-trace.sh status"
        echo "    /opt/Trace/start-trace.sh stop"
        echo ""
        if command -v systemctl &>/dev/null; then
            echo "    # 或使用 systemd"
            echo "    sudo systemctl start trace"
            echo "    sudo systemctl enable trace  # 開機自啟"
            echo ""
        fi
        ...
        echo ""
        echo "    # 直接啟動（桌面環境）"
        echo "    Trace --no-sandbox"
        echo ""
        echo "    # 或從應用程式選單尋找 Trace"
        echo ""
    fi

    echo -e "  ${BOLD}📖 文件:${NC}  https://github.com/drewsephski/trace-ai"
    echo -e "  ${BOLD}🐛 回報:${NC}  https://github.com/drewsephski/trace-ai/issues"
    echo ""
