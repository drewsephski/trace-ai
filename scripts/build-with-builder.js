...
/**
 * Simplified build script for Trace
...
try {
  ...
  if (process.platform === 'win32' && builderArgs.includes('--win')) {
    ...
    if (!cleaned) {
      const traceRunning = isProcessRunningWindows('Trace.exe');
      const electronRunning = isProcessRunningWindows('electron.exe');
      if (traceRunning || electronRunning) {
        console.log('⚠️  Detected running Trace/Electron process. Attempting to close...');
        killWindowsProcesses(['Trace.exe', 'electron.exe']);
        cleaned = tryRemoveDir(winUnpackedDir);
        if (!cleaned) {
          console.log('⚠️  Directory still locked. Please close any running Trace/Electron processes and retry.');
        }
      }
    }
  }
  ...
  try {
    ...
    const winExePath = path.join(outDir, 'win-unpacked', 'Trace.exe');
    ...
    console.log('⚠️  Windows local build failed after Trace.exe was produced.');
    if (firstError) {
