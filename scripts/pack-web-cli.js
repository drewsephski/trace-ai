...
const tarballName = `trace-web-${version}-${normalizedPlatform}-${normalizedArch}.tar.gz`;
...
const tarballContentDir = path.join(stagingDir, 'trace-web');
...
const executableName = platform === 'win32' ? 'trace-web.exe' : 'trace-web';
...
// `trace-web version` match the tarball filename.
...
execSync(`tar -czf ${path.basename(tarballPath)} -C ${stagingDir} trace-web`, {
