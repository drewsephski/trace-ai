...
const KILLABLE_NAMES = new Set(['electron', 'trace', 'trace.exe']);
...
function listLikelyConflictingProcesses() {
  try {
    if (isWindows()) {
      const output = run(
        "powershell -NoProfile -Command \"Get-Process | Where-Object { $_.ProcessName -in @('electron','Trace','node','bun') } | Select-Object ProcessName,Id | ConvertTo-Json -Compress\""
      );
      const parsed = output ? JSON.parse(output) : [];
      return Array.isArray(parsed) ? parsed : [parsed];
    }

    const output = run(`ps -A -o pid=,comm= | egrep "electron|Trace|node|bun" || true`);
    return output
      .split(/\r?\n/)
      .filter(Boolean)
      .map((line) => {
...
function cleanupByName() {
  ...
  for (const proc of processes) {
    ...
    if (!['electron', 'trace'].some((k) => rawName.includes(k))) continue;
    if (killPid(pid)) {
...
function launch(scriptName, withExtensions) {
  ...
  if (withExtensions) {
    env.TRACE_EXTENSIONS_PATH = path.resolve(process.cwd(), 'examples');
    log(`TRACE_EXTENSIONS_PATH=${env.TRACE_EXTENSIONS_PATH}`);
  }
