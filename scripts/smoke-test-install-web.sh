...
export INSTALL_DIR="/tmp/trace-web-smoke-test"
...
if [[ ! -x "${INSTALL_DIR}/trace-web" ]]; then
    echo "❌ CLI executable not found or not executable: ${INSTALL_DIR}/trace-web"
    exit 1
fi
echo "✓ CLI executable exists"

if [[ ! -L "${BIN_DIR}/trace-web" ]]; then
    echo "❌ Symlink not found: ${BIN_DIR}/trace-web"
    exit 1
fi
...
VERSION_OUTPUT=$(trace-web version 2>&1 || echo "")
