...
async function main() {
  ...
  if (shouldClean) {
    await killProcessByName('Trace.exe');
    await killProcessByName('Trace');
  ...
  const env = {
    ...process.env,
    TRACE_EXTENSIONS_PATH: path.join(projectRoot, 'examples'),
  };

  console.log(`[packaged-launch] executable: ${packaged.executablePath}`);
  console.log(`[packaged-launch] cwd: ${packaged.cwd}`);
  console.log(`[packaged-launch] TRACE_EXTENSIONS_PATH: ${env.TRACE_EXTENSIONS_PATH}`);
