 ...
 *  1. TRACE_BACKEND_RUN_ID env (download from AionCore Manual Build artifact)
 *  2. TRACE_BACKEND_VERSION env (for ad-hoc release overrides)
 *  3. "aioncoreVersion" field in repo-root package.json (the pin)
 *  4. 'latest' (fallback; not recommended for reproducible builds)
 *
 * Environment variables:
 *  - TRACE_BACKEND_RUN_ID: AionCore Manual Build workflow run id
 *  - TRACE_BACKEND_VERSION: override the pinned version
 *  - TRACE_BACKEND_ARCH: target architecture (default: process.arch)
...
// Support cross-compilation: TRACE_BACKEND_ARCH > npm_config_target_arch > process.arch
const arch = process.env.TRACE_BACKEND_ARCH || process.env.npm_config_target_arch || process.arch;
