...
export const UPDATE_AVAILABLE_EVENT = 'trace-update-available';
...
const RELEASES_PAGE_URL = 'https://github.com/drewsephski/trace-ai/releases';
...
export const useUpdateNotificationController = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('trace-open-update-modal', handleWindowOpen);
    ...
    return () => {
      removeOpenListener();
      window.removeEventListener('trace-open-update-modal', handleWindowOpen);
