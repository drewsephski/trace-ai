...
const WebuiModalContent: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribe = webui.statusChanged.on((data) => {
      ...
      if (data.running) {
        setStatus((prev) => ({
          ...(prev || { adminUsername: DEFAULT_ADMIN_USERNAME }),
          running: true,
  ...
  const handleResetUsername = () => {
    usernameForm.setFieldsValue({
      newUsername: status?.adminUsername || DEFAULT_ADMIN_USERNAME,
    });
  ...
  const displayUsername = status?.adminUsername || DEFAULT_ADMIN_USERNAME;
                  ...
                  onClick={() =>
                    void talkToButler({
                      prompt: t('settings.talkToButler.prompt.setupRemote', {
                        defaultValue:
                          'Help me set up remote access so I can open Trace from my phone or over the internet.',
                      }),
                    })
                  }
                >
