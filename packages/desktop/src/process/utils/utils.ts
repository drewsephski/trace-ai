...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'trace');
};
 ...
 * Release builds use ~/.trace; dev builds use ~/.trace-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.trace，Dev 模式使用 ~/.trace-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, 'trace');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.trace'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.trace-config; dev builds use ~/.trace-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.trace-config，Dev 模式使用 ~/.trace-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.trace-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[Trace] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[Trace] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[Trace] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[Trace] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
