...
function ensureSystemUser(db: ISqliteDriver): void {
  ...
  ).run(DEFAULT_USER_ID, DEFAULT_USERNAME, DEFAULT_PASSWORD_PLACEHOLDER, now, now);
  db.prepare(
    `UPDATE users
     SET username = ?, updated_at = ?
     WHERE id = ? AND username IN ('system_default_user', 'admin', 'system')`
  ).run(DEFAULT_USERNAME, now, DEFAULT_USER_ID);
}
