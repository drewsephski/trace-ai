import { startWebHost, startStaticServer } from '@trace/web-host';
import type { WebHostHandle, StaticServerHandle } from '@trace/web-host';
...
// tarball layout:
//   trace-web/
//   ├── trace-web              ← bun-compiled standalone binary (process.execPath)
...
const isPackaged = (() => {
  ...
  return exeName === 'trace-web' || exeName === 'trace-web.exe';
})();

const BACKEND_BINARY = process.platform === 'win32' ? 'aioncore.exe' : 'aioncore';
const DEFAULT_PORT = 25808;
const RESET_COMMAND = isPackaged ? 'trace-web resetpass' : 'bun run resetpass';
...
function resolveBackendBinary(flags: Map<string, string | true>): string {
  ...
  const envOverride = process.env.TRACE_BACKEND_BIN;
...
function resolveDataDir(flags: Map<string, string | true>): string {
  ...
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.TRACE_DATA_DIR;
  if (envOverride) return path.resolve(envOverride);
  return path.join(os.homedir(), '.trace-web');
}

function resolveLogDir(flags: Map<string, string | true>, dataDir: string): string {
  const override = flags.get('log-dir');
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.TRACE_LOG_DIR;
  if (envOverride) return path.resolve(envOverride);
...
function resolveAllowRemote(flags: Map<string, string | true>): boolean {
  ...
  const env = process.env.TRACE_ALLOW_REMOTE ?? process.env.TRACE_REMOTE;
  if (!env) return false;
...
async function runStart(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(staticDir)) {
    console.error(`[trace-web] static dir not found: ${staticDir}`);
    console.error(`  hint: pass --static-dir <path> pointing to the SPA build output`);
    process.exit(1);
  }

  console.log(`[trace-web] version    : ${version}`);
  console.log(`[trace-web] data dir   : ${dataDir}`);
  console.log(`[trace-web] log dir    : ${logDir}`);
  console.log(`[trace-web] static dir : ${staticDir}`);
  console.log(`[trace-web] backend bin: ${backendBin}`);
  console.log(`[trace-web] launching  : port=${port} allowRemote=${allowRemote}`);
  if (allowRemote) {
    console.warn(
      '[trace-web] remote mode is private-network experimental; do not expose this port directly to the public internet.'
    );
    console.warn(
      '[trace-web] for public access, keep WebUI on loopback behind an authenticated HTTPS proxy, VPN, or SSH tunnel.'
    );
  }
  ...
  if (!backendAvailable) {
    ...
    console.warn('   To enable backend: download aioncore and set TRACE_BACKEND_BIN.');
    console.warn('');
    ...
    console.log('');
    console.log('Trace WebUI (frontend only) is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
    if (autoOpenBrowser) {
      const openResult = openBrowserUrl(handle.localUrl);
      if (openResult.ok) {
        console.log(`[trace-web] opened ${handle.localUrl} in your browser.`);
      } else {
        console.warn(`[trace-web] could not open the browser automatically: ${openResult.reason}`);
      }
    }
    console.log('');
    ...
    console.log('');
    console.log('Trace WebUI is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
  ...
  const shutdown = async (signal: string): Promise<void> => {
    ...
    shuttingDown = true;
    console.log(`\n[trace-web] received ${signal}, stopping...`);
    try {
      if (currentHandle) await currentHandle.stop();
    } catch (err) {
      console.error('[trace-web] stop failed:', err);
    }
    process.exit(0);
  };
...
/**
 * `trace-web resetpass` — spin up the backend just long enough to POST
...
async function runResetPassword(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(backendBin)) {
    console.error(`[trace-web] backend binary not found: ${backendBin}`);
    console.error('  hint: pass --backend-bin <path> or set TRACE_BACKEND_BIN');
    process.exit(1);
  }
  const dataDir = resolveDataDir(flags);
  fs.mkdirSync(dataDir, { recursive: true });
  ...
  console.log(`[trace-web] resetting admin password in ${dataDir}`);
  ...
  try {
    ...
    if (!ready) {
      console.error('[trace-web] backend did not become ready within 15s');
      process.exit(1);
    }
    ...
    if (!res.ok) {
      console.error(`[trace-web] /api/webui/reset-password returned ${res.status}`);
      process.exit(1);
    }
    ...
    const username = payload.data?.username ?? payload.username ?? 'Drew';
    if (!newPassword) {
      console.error('[trace-web] reset-password response missing new_password');
      process.exit(1);
    }
    console.log(`[trace-web] username: ${username}`);
    console.log(`[trace-web] new password: ${newPassword}`);
    console.log('[trace-web] existing sessions have been invalidated.');
  } finally {
    try {
      await handle.stop();
    } catch {
...
async function main(): Promise<void> {
  ...
  if (command === '--help' || command === 'help' || command === '-h') {
    console.log(`Usage: trace-web <command> [options]
...
Options for start:
  ...
  --remote                Bind 0.0.0.0 for private LAN/VPN clients only
  --open                  Force opening the local URL in a browser
  --no-open               Disable automatic browser opening
  --data-dir <path>       Override data dir (default: ~/.trace-web)
...
Options for resetpass:
  --data-dir <path>       Which data dir to reset (default: ~/.trace-web)
  --backend-bin <path>    Override backend binary path

Environment variables:
  TRACE_PORT, TRACE_ALLOW_REMOTE, TRACE_DATA_DIR, TRACE_LOG_DIR,
  TRACE_BACKEND_BIN, TRACE_OPEN_BROWSER
`);
    return;
  }
  ...
  if (command !== 'start') {
    ...
    console.error('Usage: trace-web [start|resetpass|version|help]');
    process.exit(1);
  }
...
main().catch((err: Error) => {
  console.error('[trace-web] fatal:', err.message);
