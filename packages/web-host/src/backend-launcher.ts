...
export type BackendLaunchOptions = {
  ...
  logDir?: string;
  /**
   * System dirs exposed to the backend via TRACE_{CACHE,WORK,LOG}_DIR env.
...
export function buildSpawnArgs(config: SpawnConfig): string[] {
  const logLevel = process.env.TRACE_LOG_LEVEL || (config.isPackaged ? 'info' : 'debug');
  const args = [
    '--port',
    String(config.port),
    '--data-dir',
...
/**
 * Backend reads TRACE_{CACHE,WORK,LOG}_DIR env vars to report system dirs
 * (see AionCore/crates/trace-system/src/sysinfo.rs). Inject them so the
 * backend's `/api/system/info` matches what Electron main persists in
 * ProcessEnv('trace.dir').
 */
export function buildSpawnEnv(dirs: BackendDirConfig): NodeJS.ProcessEnv {
  return {
    ...process.env,
    TRACE_CACHE_DIR: dirs.cacheDir,
    TRACE_WORK_DIR: dirs.workDir,
    TRACE_LOG_DIR: dirs.logDir,
  };
}
