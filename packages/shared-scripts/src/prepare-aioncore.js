 ...
 *
 * Resolution order:
 *  1. GitHub Actions artifact download when TRACE_BACKEND_RUN_ID is set
...
function prepareManagedResources(binaryPath, targetDir) {
  ...
  execFileSync(binaryPath, ['--data-dir', dataDir, 'prepare-managed-resources', '--bundle-out', bundleOut], {
    ...
    env: {
      ...process.env,
      TRACE_BUNDLED_MANAGED_RESOURCES: '',
    },
  });
...
function prepareAioncore(options) {
  ...
  const actionsRunId = (process.env.TRACE_BACKEND_RUN_ID || '').trim();
